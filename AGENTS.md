...
所有仓库内的源代码注释（包括 Go、TypeScript、JavaScript、SQL 及其他语言）必须使用**中文**编写，包括包注释、文件注释、函数注释、类型注释、常量注释、变量注释和行内关键逻辑注释。以下情况除外：

- 代码生成器、框架脚手架或外部工具自动生成的注释，保持生成内容不变。
- 引用外部 API、第三方库或协议规范时，专有名词、字段名、路径、URL 等保持原文，但说明性描述仍用中文。
- 测试文件中引用测试框架约定的固定英文字符串保持不变，说明部分用中文。

新增或修改任何源码文件时，必须确认文件内注释语言符合本规范；审查时发现英文注释必须作为问题报告（警告级）。
