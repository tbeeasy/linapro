...
Moka HCM OpenAPI 客户端插件，提供 `HCMAuth` 认证（Basic Auth + MD5withRSA 查询参数签名）、HTTP 传输客户端、`GetReportData` 报表接口，以及供下游插件通过能力依赖获取的客户端工厂。

实现位置：`apps/lina-plugins/linapro-moka-hcm/`，核心包 `backend/hcm/`（`auther.go`、`sign.go`、`client.go`、`report.go`）。
...
4. 将 `entCode`、`apiCode`、`nonce`、`timestamp`、`sign` 以 URL 查询参数追加到请求 URL，不覆盖已有查询参数

#### Scenario: 成功追加签名参数
- **WHEN** 调用 `ApplyAuth` 且请求 URL 无已有查询参数
- **THEN** 请求 URL 查询字符串包含 `entCode`、`apiCode`、`nonce`、`timestamp`、`sign` 共五个参数
...
- 向 `/api-platform/hcm/oapi/v1/report/getReportData` 发送 POST 请求，请求体 `{"reportId": <reportID>}`
- 响应 `code` 为 200 或 1000000 时视为成功
- 成功时返回解析后的 `*ReportData`（含 `Headers`、`Rows`、`Size`）
- 响应 `data` 为 null 时返回空 `&ReportData{}`，不返回 error
...
- **THEN** 返回包含 code 和 msg 的 error，`*ReportData` 为 nil
...
插件 SHALL 在 LinaPro 插件生命周期完成：
- `OnStart` 钩子从插件配置服务读取 `api_key`、`api_code`、`ent_code`、`private_key_pem` 四个必填项
- 解析 PEM 格式 RSA 私钥（支持 PKCS#1 和 PKCS#8），构造 `*hcm.Client`
- 将 `*hcm.Client` 注册为插件能力，供依赖插件通过能力接口获取
- 任意必填项缺失或私钥解析失败时，`OnStart` SHALL 返回 error，插件不完成启动

#### Scenario: 配置完整时插件正常启动
- **WHEN** 四个必填项均存在且 PEM 私钥格式正确
...
`Client` SHALL 提供员工列表能力，封装 Moka `POST /api-platform/hcm/oapi/v2/batch/data` 接口，供员工目录同步消费。
...
- `auther.go`：`Auther` 接口、`HCMCredential`、`HCMAuth`
- `client.go`：`Client` 及 `PostJSON`/`GetJSON` 方法
- `sign.go`：`Sign`、`BuildQuery`、`nonce` 等签名工具
- `report.go`：`GetReportData` 及响应类型
