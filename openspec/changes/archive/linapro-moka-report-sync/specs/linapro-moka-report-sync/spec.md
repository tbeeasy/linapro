...
定时把 Moka 报表中心数据（`GetReportData`）同步到飞书多维表格（Bitable），采用 `uniqueField` 主键（默认 `工号`）+ 交集列完整性门闩的幂等写入策略。

实现位置：`apps/lina-plugins/linapro-moka-report-sync/`，核心包：
- `backend/internal/syncer/`：纯算法（`flatten.go`、`plan.go`、`plan_test.go`），无 I/O 依赖
- `backend/internal/feishu/`：Bitable 读写（`bitable.go`）
- `backend/internal/service/`：编排（`sync.go`）
- `backend/internal/config/`：配置加载（`config.go`）
- `backend/plugin.go`：插件入口，注册定时 job
...
对每个 Moka 报表行：无同键记录 SHALL 新增整行；有同键记录且「Moka 报表列 ∩ Bitable 表字段」交集内存在空字段 SHALL 用 Moka 最新值覆盖更新整条；交集内无空字段 SHALL 跳过（冻结）。完整性判断 SHALL 仅针对交集列，不涉及 Bitable 表中报表未声明的其他列。Bitable 字段列表 SHALL 通过字段 API 获取（权威 schema），不依赖现有记录值。
